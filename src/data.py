...
from utils import rgb_to_lab_norm, rgb_to_ycbcr
...
class MinecraftDF2KDataModule(DataModuleBase):
    def __init__(self, train_dirs, test_dirs, random_crop, ycbcr, batch_size=64, num_workers=4,
    ...
    def setup(self, stage=None):
        train_dataset = ConcatDataset([
            ImageFolder(self.train_dirs[0], transform=self.transform),
            ImageFolder(self.train_dirs[1], transform=self.transform)
        ])
        val_dataset = ConcatDataset([
            ImageFolder(self.train_dirs[0], transform=self.val_transform),
            ImageFolder(self.train_dirs[1], transform=self.val_transform),
        ])
        test_dataset = ConcatDataset([
            ImageFolder(self.test_dirs[0], transform=self.transform),
            ImageFolder(self.test_dirs[1], transform=self.transform)
        ])
