...
class DownBranch(nn.Module):
    def __init__(self, in_channels):
        ...
        N = [32, 32, 32, 64, 64, 64, 64, 64, 32]

        def downsampling_unit(in_c, out_c1, out_c2, out_c3):
            return nn.Sequential(
                nn.Conv2d(in_c, out_c1, kernel_size=3, stride=1, padding=1),
                nn.PReLU(out_c1),
                nn.Conv2d(out_c1, out_c2, kernel_size=3, stride=1, padding=1),
                nn.PReLU(out_c2),
                nn.Conv2d(out_c2, out_c3, kernel_size=3, stride=2, padding=1),
                nn.PReLU(out_c3),
            )

        self.net = nn.Sequential(
            downsampling_unit(in_channels, N[0], N[1], N[2]),
            downsampling_unit(N[2], N[3], N[4], N[5]),
            downsampling_unit(N[5], N[6], N[7], N[8]),
        )
...
class UpBranch(nn.Module):
    def __init__(self, in_channels):
        ...
        N = [32, 32, 32, 64, 64, 64, 64, 64, 32][::-1]

        def upsampling_unit(in_c, out_c1, out_c2, out_c3):
            return nn.Sequential(
                nn.ConvTranspose2d(in_c, out_c1, kernel_size=4, stride=2, padding=1),
                nn.PReLU(out_c1),
                nn.Conv2d(out_c1, out_c2, kernel_size=3, stride=1, padding=1),
                nn.PReLU(out_c2),
                nn.Conv2d(out_c2, out_c3, kernel_size=3, stride=1, padding=1),
                nn.PReLU(out_c3),
            )

        self.net = nn.Sequential(
            upsampling_unit(in_channels, N[0], N[1], N[2]),
            upsampling_unit(N[2], N[3], N[4], N[5]),
            upsampling_unit(N[5], N[6], N[7], N[8]),
        )
...
class DCAL_triple(pl.LightningModule):
    def __init__(self, learning_rate=1e-3):
        super().__init__()
        self.save_hyperparameters()
        self.name = "DCAL_triple"
