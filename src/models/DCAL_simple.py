...
class DownBranch(nn.Module):
    def __init__(self, in_channels):
        ...
        def downsampling_unit(in_c, out_c1):
            return nn.Sequential(
                nn.Conv2d(in_c, out_c1, kernel_size=3, stride=2, padding=1),
                nn.PReLU(out_c1),
            )

        self.net = nn.Sequential(
            downsampling_unit(in_channels, N[0]),
            downsampling_unit(N[0], N[1]),
            downsampling_unit(N[1], N[2]),
        )
...
class UpBranch(nn.Module):
    def __init__(self, in_channels):
        ...
        def upsampling_unit(in_c, out_c1):
            return nn.Sequential(
        ...
        self.net = nn.Sequential(
            upsampling_unit(in_channels, N[0]),
            upsampling_unit(N[0], N[1]),
            upsampling_unit(N[1], N[2]),
        )
...
class DCAL_simple(pl.LightningModule):
    def __init__(self, learning_rate=1e-3):
        super().__init__()
        self.save_hyperparameters()
        self.name = "DCAL_simple"
